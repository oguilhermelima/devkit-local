...
command_tv() {
  ...
  case "$operation" in
    connect)
      ip="${1:-}"
      [ -n "$ip" ] || { devkit_error "Usage: devkit tv connect <ip> [--port 5555]"; return "$DEVKIT_USAGE_ERROR"; }
      shift
      while [ "$#" -gt 0 ]; do
        arg="$1"
        case "$arg" in
          --port) port="${2:-}"; shift 2 ;;
          -h|--help) printf 'Usage: devkit tv connect <ip> [--port 5555]\n'; return 0 ;;
    ...
    disconnect)
      ...
      if [ "$#" -gt 0 ]; then
        shift
        [ "$#" -eq 0 ] || { devkit_error "Usage: devkit tv disconnect [<ip>]"; return "$DEVKIT_USAGE_ERROR"; }
        module_tv_adb_doctor >/dev/null || return 1
        adb disconnect "$ip"
      else
        module_tv_adb_doctor >/dev/null || return 1
      ...
      ;;
    -h|--help|"") printf 'Usage: devkit tv connect <ip> [--port 5555] | devkit tv disconnect [<ip>]\n' ;;
