...
megabrain_skill_stamp_path() {
  local target="$1" key=''
  key="$(megabrain_sha256_text "$target")" || return 1
...
megabrain_skill_write_stamp() {
  local target="$1" source_hash="$2" target_hash="$3" mtime="$4" size="$5"
  local stamp='' temp=''
  ...
  if ! printf '%s\n%s\n%s\n%s\n' "$source_hash" "$target_hash" "$mtime" "$size" >"$temp"; then
    rm -f "$temp"
    return 1
  fi
  mv -f "$temp" "$stamp"
...
megabrain_skill_target_reconcile() {
  local source="$1" source_hash="$2" target="$3" repair="$4"
  local target_hash='' target_mtime='' target_size='' stamp=''
  local stamp_source='' stamp_target='' stamp_mtime='' stamp_size='' temp=''
  MEGABRAIN_SKILL_TARGET_RESULT=error
  MEGABRAIN_SKILL_TARGET_ERROR=""

  target_mtime="$(megabrain_path_mtime "$target" 2>/dev/null || true)"
  target_size="$(megabrain_skill_file_size "$target" 2>/dev/null || true)"
  stamp="$(megabrain_skill_stamp_path "$target" 2>/dev/null || true)"
  if [ -n "$stamp" ] && [ -f "$stamp" ]; then
    stamp_source="$(sed -n '1p' "$stamp" 2>/dev/null || true)"
    stamp_target="$(sed -n '2p' "$stamp" 2>/dev/null || true)"
    stamp_mtime="$(sed -n '3p' "$stamp" 2>/dev/null || true)"
    stamp_size="$(sed -n '4p' "$stamp" 2>/dev/null || true)"
    if [ "$stamp_source" = "$source_hash" ] && [ "$stamp_target" = "$source_hash" ] &&
      [ "$stamp_mtime" = "$target_mtime" ] && [ "$stamp_size" = "$target_size" ]; then
  ...
  if [ "$target_hash" = "$source_hash" ]; then
    if ! megabrain_skill_write_stamp "$target" "$source_hash" "$target_hash" "$target_mtime" "$target_size"; then
  ...
  target_mtime="$(megabrain_path_mtime "$target" 2>/dev/null || true)"
  target_size="$(megabrain_skill_file_size "$target" 2>/dev/null || true)"
  if ! megabrain_skill_write_stamp "$target" "$source_hash" "$source_hash" "$target_mtime" "$target_size"; then
    MEGABRAIN_SKILL_TARGET_ERROR="could not write skill sync stamp for: $target"
    printf 'megabrain: %s\n' "$MEGABRAIN_SKILL_TARGET_ERROR" >&2
    return 1
  fi
...
megabrain_skill_scan() {
  local repair="$1" source='' source_hash='' target='' scan_rc=0
  MEGABRAIN_SKILL_TARGET_COUNT=0
  MEGABRAIN_SKILL_DRIFT_COUNT=0
  MEGABRAIN_SKILL_FAILURE_COUNT=0
  MEGABRAIN_SKILL_REPAIRED_COUNT=0
  ...
  source_hash="$(megabrain_skill_hash_file "$source" 2>/dev/null || true)"
  ...
  while IFS= read -r target; do
    ...
    megabrain_skill_target_reconcile "$source" "$source_hash" "$target" "$repair" || scan_rc=1
