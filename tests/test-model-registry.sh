...
assert_equal "$(printf '%s' "$list_output" | jq -r '.models[] | select(.agent == "codex") | .provenance.kind' | head -n 1)" curated
printf 'registry provenance: live agy and curated codex\n'
...
"$root/devkit" chain repair legacy --step 1 --model gemini-3.8-flash-high --effort high >/dev/null
