...
# WHY: three variables are what it takes to run megabrain without touching anything the
...
# and module state; MEGABRAIN_FACTS_FILE covers the fact store, which otherwise lives in
# the installation root and is a file people commit. Two probes written by hand today set
# only the state directory and wrote into the repository and into the operator's own
# dispatch directory, so the completeness of this set is worth pinning rather than
# remembering.
mkdir -p "$sandbox/home" "$sandbox/state"
run_sandboxed() {
  env HOME="$sandbox/home" \
    MEGABRAIN_STATE_DIR="$sandbox/state" \
    MEGABRAIN_FACTS_FILE="$sandbox/facts.json" \
    "$root/megabrain" "$@"
}
...
# Reads first, then the writes that actually create state. fact add and model add are here
# by name: those are the two that escaped a hand-written sandbox today.
...
run_sandboxed fact list --json >/dev/null 2>&1 || true
run_sandboxed fact add sandbox-probe --measurement 'written inside the sandbox' \
  --who tester --when '2026-09-08T12:00:00Z' --command 'echo' >/dev/null 2>&1 || true
...
[ -f "$sandbox/facts.json" ] || fail 'the sandboxed fact was not written inside the sandbox'
grep -q sandbox-probe "$sandbox/facts.json" || fail 'the sandboxed fact store has no fact'
...
printf 'ok: HOME, MEGABRAIN_STATE_DIR and MEGABRAIN_FACTS_FILE isolate a run completely\n'
