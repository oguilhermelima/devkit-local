...
[ -n "${DEVKIT_SESSION_ID:-}" ] || megabrain_hook_finish
devkit_dispatch_find_child >/dev/null 2>&1 || megabrain_hook_finish

MEGABRAIN_HOOK_DISPATCH="$DEVKIT_FOUND_DISPATCH"
...
devkit_dispatch_message_append "$MEGABRAIN_HOOK_DISPATCH" child stalled "$MEGABRAIN_HOOK_TEXT" "$DEVKIT_SESSION_ID" >/dev/null 2>&1 || megabrain_hook_finish
